...
import { type KpiEvalConfig, bonusTierFromPercentile, getDefaultKpiEvalConfig, kpiDayKeyFromTs, kpiMonthKeyFromTs } from './kpiEvaluationConfig.js'

export { kpiDayKeyFromTs, kpiMonthKeyFromTs }

export const VALID_CALL_MIN_BILL_SECONDS = 30
export const VALID_CALL_DEDUP_WINDOW_MS = 3 * 60 * 60 * 1000

export const COLLECTIONS_EXTRA = {
  leadEvents: 'leadEvents',
  kpiMonthly: 'kpiMonthly',
  kpiLeadEvents: 'kpiLeadEvents',
  kpiLeadTagDay: 'kpiLeadTagDay',
...
function leadTagDayKey(counselorUid: string, leadId: string, day: string, tag: 'WARM' | 'HOT'): string {
  return `${counselorUid}_${leadId}_${day}_${tag}`
}

function tagFieldForIncrement(type: string, from: string, to: string): 'warmNew' | 'hotNew' | null {
  if (type !== 'TAG_CHANGED') return null
  if (to === 'WARM' && from !== 'WARM' && from !== 'HOT') return 'warmNew'
  if (to === 'HOT' && from !== 'HOT') return 'hotNew'
  return null
}
...
export async function processLeadEventDoc(
  ...
  await db.runTransaction(async (tx) => {
    ...
    const day = kpiDayKeyFromTs(at)
    const teamLeadUid = data.teamLeadUid ? String(data.teamLeadUid) : null
    const tagField = tagFieldForIncrement(type, from, to)
    if (tagField) {
      const tag = tagField === 'warmNew' ? 'WARM' : 'HOT'
      const dedupRef = db.collection(COLLECTIONS_EXTRA.kpiLeadTagDay).doc(leadTagDayKey(counselorUid, leadId, day, tag))
      const dedupSnap = await tx.get(dedupRef)
      if (dedupSnap.exists) {
        tx.set(processedRef, { skipped: true, reason: 'tag_dedup_day', processedAt: Timestamp.now() })
        tx.set(eventRef, { kpiAppliedAt: Timestamp.now() }, { merge: true })
        return
      }
      const fieldIncrements = { [tagField]: FieldValue.increment(1), updatedAt: Timestamp.now() }
      tx.set(
        db.collection(kpiDailyCol).doc(day).collection('counselors').doc(counselorUid),
        { date: day, counselorUid, teamLeadUid, ...fieldIncrements },
        { merge: true },
      )
      if (teamLeadUid) {
        tx.set(
          db.collection(kpiDailyCol).doc(day).collection('teams').doc(teamLeadUid),
          { date: day, teamLeadUid, ...fieldIncrements },
          { merge: true },
        )
      }
      tx.set(dedupRef, { counselorUid, leadId, day, tag, processedAt: Timestamp.now() })
