...
function toCallWireFromOmicallDoc(id: string, data: Record<string, unknown>): OmicallCallWire {
  return {
    ...
    recordSeconds: num(data.recordSeconds),
    recordingFileUrl: str(data.recordingFileUrl) || undefined,
    outcome: str(data.outcome) === 'CONNECTED' ? 'CONNECTED' : str(data.outcome) === 'NO_ANSWER' ? 'NO_ANSWER' : 'OTHER',
...
function toCallWireFromInteractionDoc(id: string, data: Record<string, unknown>): OmicallCallWire | null {
  ...
  const outcome: CallOutcome = callOutcome === 'CONNECTED' ? 'CONNECTED' : callOutcome === 'NO_ANSWER' ? 'NO_ANSWER' : 'OTHER'
  return {
    id: `int-${id}`,
...
function scopeAllowsWireCall(
  ...
  if (isAdminLikeRole(caller.role)) {
    ...
    if (requestedScope.mode === 'team' && requestedScope.teamLeadUid) return call.teamLeadUid === requestedScope.teamLeadUid
    return true
  }
  if (caller.role === 'team_lead') {
    if (requestedScope.mode === 'global') return false
    if (requestedScope.mode === 'team' && requestedScope.teamLeadUid && requestedScope.teamLeadUid !== caller.id) return false
...
/** Fallback đọc cuộc gọi qua Admin SDK khi client bị chặn bởi rules/index. */
export const fetchOmicallCallsForClient = onCall(async (request) => {
  ...
  let rows: OmicallCallWire[] = callSnap.docs.map((d) => toCallWireFromOmicallDoc(d.id, d.data() as Record<string, unknown>))
  ...
  const scoped = rows
    ...
    .sort((a, b) => (b.endedAtMs || b.startedAtMs || b.createdAtMs || 0) - (a.endedAtMs || a.startedAtMs || a.createdAtMs || 0))
    .slice(0, maxRows)
