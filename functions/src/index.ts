...
import { isLeadChamCall } from './kpiCallMetrics.js'
import { isOnOrAfterGoLive, loadKpiV2Config } from './kpiV2Config.js'
import {
  applyValidCallKpi,
  evaluateValidCall,
  kpiMonthKeyFromTs,
...
async function updateDailyKpi(
  ...
  const v2Active = isOnOrAfterGoLive(day, kpiV2.goLiveDate)
  const billSec = Math.max(call.billSeconds || call.answerSeconds || 0, 0)
  const connected = call.outcome === 'CONNECTED'
  const leadChamInc =
    v2Active && isLeadChamCall(connected, billSec, {
      leadChamMinSeconds: kpiV2.leadChamMinSeconds,
      leadChamMaxExclusive: kpiV2.leadChamMaxSecondsExclusive,
    })
      ? 1
      : 0
  const increments = {
    ...
    connectedCalls: FieldValue.increment(connected ? 1 : 0),
    missedCalls: FieldValue.increment(connected ? 0 : 1),
    talkSeconds: FieldValue.increment(billSec),
    ringSeconds: FieldValue.increment(Math.max(call.durationSeconds - billSec, 0)),
    recordings: FieldValue.increment(call.recordingFileUrl ? 1 : 0),
    ...(leadChamInc ? { leadCham: FieldValue.increment(leadChamInc) } : {}),
...
function financeIncrements(slot: string, amount: number, lpxtMinVnd: number, v2Active: boolean) {
  const isDeposit = slot === 'deposit'
  const isLpxt =
    v2Active &&
    !isDeposit &&
    LPXT_SUPPLEMENT_SLOTS.includes(slot as (typeof LPXT_SUPPLEMENT_SLOTS)[number]) &&
    amount >= lpxtMinVnd
  return {
    depositPaidCount: FieldValue.increment(isDeposit ? 1 : 0),
    lpxtCount: FieldValue.increment(isLpxt ? 1 : 0),
  ...
  for (const leadDoc of leadSnap.docs) {
    ...
    for (const slot of PAYMENT_KEYS) {
      ...
      await db.runTransaction(async (tx) => {
        ...
        const increments = financeIncrements(slot, amount, kpiV2.lpxtMinVnd, v2Active)
        tx.set(
...
export const omicallSyncMyExtension = onCall(
  ...
  async (request) => {
    ...
    const patch: Record<string, unknown> = {
      ...
      omicallAgentId: user.omicallAgentId || null,
      omicallOutboundNumber: outbound || null,
      omicallSyncedAt: Timestamp.now(),
      updatedAt: Timestamp.now(),
    }
