...
describe('defaultPermissionsForRole', () => {
  ...
  it('counselor: own leads, ai:use when flagged by manager', () => {
  ...
  it('ctv: field staff without peer reassign', () => {
    const perms = defaultPermissionsForRole('ctv')
    expect(hasPermission(perms, 'dashboard:counselor')).toBe(true)
    expect(hasPermission(perms, 'leads:reassign:peer')).toBe(false)
    expect(hasPermission(perms, 'ai:use')).toBe(false)
  })

  it('team_lead: team scope, playbooks, ai:use when flagged by manager', () => {
