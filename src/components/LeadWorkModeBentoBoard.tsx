...
type Props = {
  ...
  className?: string
  /**
   * true = chưa bật quét toàn phạm vi (đếm trên trang đang xem).
   * false = đã quét / đang lọc chế độ trên toàn phạm vi (đã giữ sẵn hồ sơ khớp).
   */
  sampleOnly?: boolean
  /** Đang tải quét fullScope sau khi chọn chế độ. */
  scanning?: boolean
}
    ...
    <div className={['space-y-1.5', className].filter(Boolean).join(' ')}>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">
          Cách làm việc — lọc danh sách
          {sampleOnly && !filtering ? ' (ước tính trang này)' : ''}
          {filtering ? ' (toàn phạm vi)' : ''}
        </p>
        <p className="text-[11px] text-slate-500" aria-live="polite">
          {scanning
            ? 'Đang quét toàn bộ danh sách trong phạm vi quyền…'
            : filtering
              ? `Đang lọc: ${leadWorkModeLabel(active)} — chỉ hiện hồ sơ khớp`
              : sampleOnly
                ? 'Số trên ô là ước tính trang này. Bấm một ô để quét và lọc toàn bộ.'
      ...
      <BentoGrid tight className="w-full !gap-2 sm:!grid-cols-2 lg:!grid-cols-4">
        ...
        >
          ...
          <p className="bento-stat__hint !text-[10px] !leading-snug">
            {filtering
              ? 'Kết quả đang lọc'
              : summary.unset > 0
                ? `Suy diễn: ${summary.unset.toLocaleString('vi-VN')}`
                : 'Đã gán chế độ'}
          </p>
        </button>
        {LEAD_WORK_MODES.map((mode) => {
          ...
          /** Khi đang lọc mode khác, đếm trên tập đã giữ khớp không còn ý nghĩa → không hiện 0 giả. */
          const countHidden = filtering && !selected
          return (
            <button
              key={mode}
              type="button"
              ...
              title={
                countHidden
                  ? `Bấm để quét toàn phạm vi · ${leadWorkModeHint(mode)}`
                  : leadWorkModeHint(mode)
              }
            >
              <p className="bento-stat__label !text-[10px]">{leadWorkModeLabel(mode)}</p>
              <p className="bento-stat__value !text-lg tabular-nums">
                {countHidden ? '—' : summary[mode].toLocaleString('vi-VN')}
              </p>
              <p className="bento-stat__hint !text-[10px] !leading-snug">
                {countHidden ? 'Bấm để lọc toàn bộ' : leadWorkModeHint(mode)}
              </p>
            </button>
          )
        })}
