...
function scoreCallPillar(
  ...
  const validRatio = m.totalCalls > 0 ? m.validCalls / m.totalCalls : 1
  const connectRatio = m.totalCalls > 0 ? m.connectedCalls / m.totalCalls : 0
  const hlScore =
...
function scoreCompliancePillar(
  ...
  let auto = 100
  const warn = evaluateKpiRowWarnings(m as CounselorKpiSummary, cfg)
  ...
  if (m.validCalls >= 20 && noteRatio < c.minNoteRatioPerValidCall) auto -= 20
...
export function computeCompositeKpiScore(
  ...
  const compR = scoreCompliancePillar(metrics, runtime, manualComplianceScore)
...
export function computeCompositeForCounselor(
  ...
  return computeCompositeKpiScore(metrics, runtime, targets, manual?.complianceScore)
}
