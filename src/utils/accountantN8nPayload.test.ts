...
describe('accountantN8nPayload', () => {
  ...
  it('builds counselor payment update body with receipt and chat text', () => {
    const finance = {
      payments: {
        deposit: {
          amountVnd: 1_500_000,
          collectedAt: '28/05/2026',
          receiptUrl: 'https://cdn.example/bill.jpg',
        },
      },
      declaredTotalVnd: 1_500_000,
    }
    const body = buildProfileFinanceUpdateWebhookBody(
      {
        ...
        counselorEmail: 'tvv@school.vn',
        changedSlots: ['deposit'],
        resetApprovalSlots: [],
      },
      { legacy: true },
    )
    expect(body.event).toBe('update_profile')
    expect(body.sub_event).toBe('counselor_payment_submitted')
