...
/** Có dữ liệu tài chính cần lưu khi tạo hồ sơ mới (chưa có bản ghi Firestore). */
export function financeDraftHasContent(draft: LeadFinanceDraft): boolean {
  if (draft.reqFullNe) return true
  for (const { key } of PAYMENT_SLOT_DEFS) {
    const row = draft.payments[key]
    if (row.pendingFile || parseAmount(row.amount) > 0 || row.collectedAt.trim()) return true
  }
  return false
}
...
export type FinanceSavePlan = {
  ...
  /** Các khoản thu vừa thay đổi (tiền / ngày / bill) — gửi n8n từng lần. */
...
export function buildFinanceSavePlan(lead: Lead, draft: LeadFinanceDraft): FinanceSavePlan {
  ...
  const changedSlots: LeadPaymentSlotKey[] = []
