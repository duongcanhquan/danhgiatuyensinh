...
import { behaviorScoreFromPicks } from './callSessionBehaviorScore'
...
function parseDimension(raw: unknown): CallEvalDimension | null {
  ...
  for (const opt of optionsRaw) {
    ...
    if (!oid || !ol) continue
    const pointsRaw = oo.points
    const points =
      pointsRaw !== undefined && pointsRaw !== null && Number.isFinite(Number(pointsRaw))
        ? Math.round(Number(pointsRaw))
        : undefined
    options.push({ id: oid.slice(0, 64), label: ol.slice(0, 120), ...(points !== undefined ? { points } : {}) })
  }
  if (!options.length) return null
  const scoringRaw = o.scoringGroup
  const scoringGroup =
    scoringRaw === 'positive' || scoringRaw === 'negative' || scoringRaw === 'process'
      ? scoringRaw
      : undefined
  return {
    id: id.slice(0, 64),
...
export function mergeCallEvaluationConfig(partial: CallEvalDimension[] | null | undefined): CallEvalDimension[] {
  ...
  const byId = new Map<string, CallEvalDimension>()
  ...
  return sortDimensions([...byId.values()])
}
...
export function evaluationRecordFromPicks(picks: CallEvalPick[]): CallSessionEvaluationRecord {
  const { behaviorScore, behaviorPointsDelta } = behaviorScoreFromPicks(picks)
  const hasBehavior = picks.some((p) => typeof p.points === 'number')
  return {
    version: CALL_EVAL_CONFIG_VERSION,
    picks,
    ...(hasBehavior ? { behaviorScore, behaviorPointsDelta } : {}),
  }
}
