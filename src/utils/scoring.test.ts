...
import type { Lead, ScoringProfile, ScoringRule } from '../types'
...
describe('evaluateLead', () => {
  ...
  it('CONTAINS danh sách trường THPT trên khối schoolTypeKey vẫn đọc highSchool trên hồ sơ', () => {
    const profile = {
      rules: [] as ScoringRule[],
      ruleBlocks: [
        {
          ...
          rows: [
            {
              ...
              value: 'THPT Phú Xuyên B, THPT Hiệp Hòa 5, THPT Chương Mỹ B',
              allocationKind: 'absolute' as const,
              allocationValue: 40,
            },
          ],
    ...
    const lead = { highSchool: 'THPT Chương Mỹ B', schoolType: '' }
    expect(evaluateLead(lead, profile).calculatedScore).toBe(40)
  })

  it('CONTAINS trường THPT trên schoolTypeKey vẫn khớp khi loại hình trường đã có giá trị', () => {
    const profile = {
      rules: [] as ScoringRule[],
      ruleBlocks: [
        {
    ...
    const lead = { highSchool: 'THPT Chương Mỹ B', schoolType: 'Liên kết' }
    expect(evaluateLead(lead, profile).calculatedScore).toBe(40)
  })
  ...
  it('leadToEvaluationRecord maps source1 into source/leadSource for scoring rules', () => {
    ...
    const profile: Pick<ScoringProfile, 'rules' | 'ruleBlocks' | 'thresholds'> = {
      ...
      thresholds: { hotMinScore: 25, warmMinScore: 15 },
    }
    const rec = leadToEvaluationRecord(lead)
    expect(rec.source).toBe('Facebook Ads')
    expect(rec.leadSource).toBe('Facebook Ads')
    const r = evaluateLead(rec, profile)
    expect(r.calculatedScore).toBe(30)
    expect(r.priorityTag).toBe('HOT')
  })
})
...
describe('evaluateLead aux scores', () => {
  const emptyProfile: Pick<ScoringProfile, 'rules' | 'ruleBlocks' | 'thresholds'> = {
    rules: [],
    ruleBlocks: [],
    thresholds: { hotMinScore: 80, warmMinScore: 50 },
  }
  ...
  it('cộng điểm thông tin khi truyền lead + infoScoreRuntime', () => {
    ...
    const infoScoreRuntime = {
      ruleSource: 'builtin' as const,
      basePoints: 10,
      capMin: 0,
      capMax: 100,
      fields: [
        {
          ...
          enabled: true,
          pointsIfMatch: 15,
          match: (l: Lead) => Boolean(l.phone?.trim()),
        },
      ],
    }
    const r = evaluateLead(leadToEvaluationRecord(lead), emptyProfile, undefined, null, {
      lead,
      infoScoreRuntime,
  ...
  it('cộng điểm hành vi mẫu khi TVV bật cờ và profile chưa có rule sig_*', () => {
    ...
    const rec = leadToEvaluationRecord(lead)
    const r = evaluateLead(rec, emptyProfile, undefined, null, {
      lead,
      includeAuxScores: true,
    })
    expect(r.calculatedScore).toBeGreaterThan(0)
  })

  it('persistedLeadScoringFields: bật cờ hành vi cập nhật điểm khi profile chưa có rule sig_*', () => {
    const lead = { id: 'l3', calculatedScore: 0, priorityTag: 'COLD' as const } as Lead
    const emptyProfile: ScoringProfile = {
      id: 'p-empty',
      profileName: 'Test',
      description: '',
      rules: [],
    ...
    const base = persistedLeadScoringFields(lead, {}, emptyProfile, undefined, null, { includeAuxScores: true })
    const withFlag = persistedLeadScoringFields(
      lead,
      { scoringSignals: { askedTuition: true } },
      emptyProfile,
      undefined,
    ...
    expect((withFlag.calculatedScore ?? 0) - (base.calculatedScore ?? 0)).toBe(
      SCORING_SIGNAL_META.askedTuition.defaultPoints,
    )
  })
})
