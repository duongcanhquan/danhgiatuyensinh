...
export function exportEvaluatedLeadsToXlsx(
  ...
  const data: EvaluatedLeadExportRow[] = rows.map((l) => {
    ...
    return {
      ...
      'Ngành quan tâm': l.majorInterest,
      'Tỉnh/Thành phố': l.region,
      'Tỉnh (chi tiết)': l.province ?? '',
      'Quận/Huyện (Hà Nội)': l.hanoiArea ?? '',
      'Trường THPT': l.highSchoolName ?? '',
      'Học lực': l.academicLevel ?? '',
      'Dự định': l.studyIntention ?? '',
      'Loại trường': l.schoolType,
