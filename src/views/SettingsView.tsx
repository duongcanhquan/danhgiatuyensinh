...
function settingsGuideBody(sub: SettingsSubTabId, ctx: SettingsAccessContext): ReactNode {
  switch (sub) {
    ...
    case 'classification':
      return (
        <>
          <p className="font-semibold text-slate-900">Phân loại HOT / WARM / COLD</p>
          <p className={`mt-1.5 ${settingsCopyMuted}`}>
            Cấu hình <strong>tỷ trọng hồ sơ vs gọi điện</strong> — điểm tổng hợp 0–100 quyết định nhãn. Lưu tại{' '}
            <code className="rounded bg-slate-100 px-1 font-mono text-[0.9em]">scoringAux/leadClassificationConfig</code>.
          </p>
        </>
      )
    case 'kpi':
      return (
        <>
          <p className="font-semibold text-slate-900">KPI Sale — cài cho đúng</p>
          <p className={`mt-1.5 ${settingsCopyMuted}`}>
            Quyết định cách tính <strong>gọi hợp lệ</strong>, <strong>cảnh báo trên Điều hành</strong>,{' '}
            <strong>điểm tháng</strong> và <strong>hạng Vàng/Bạc/Đồng</strong>. Bấm nút{' '}
            <strong>Hướng dẫn</strong> góc phải khối bên dưới để xem chi tiết từng bước.
          </p>
          <ol className={`mt-3 list-inside list-decimal space-y-2 ${settingsCopyMuted}`}>
            <li>
              <strong>Gọi HL:</strong> giây tối thiểu + không trùng lead (giờ). TVV phải gọi từ hồ sơ.
            </li>
            <li>
              <strong>Cảnh báo:</strong> ngưỡng spam / chưa cọc / bắt máy — hiện trên màn Điều hành.
            </li>
            <li>
              <strong>Điểm tháng:</strong> mục tiêu từng chỉ số; xem điểm mẫu trước khi Lưu.
            </li>
            <li>
              <strong>Kế toán:</strong> chuỗi trạng thái duyệt cọc và Full NE phải khớp đúng chữ trên hồ sơ.
            </li>
          </ol>
          <p className={`mt-3 ${settingsCopyMuted}`}>
            Sau Lưu, số cập nhật dần (~15 phút). Xem thêm{' '}
            <Link to="/huong-dan?section=kpi-sale" className="font-semibold text-sky-800 underline">
              Hướng dẫn sử dụng → KPI Sale
            </Link>
            .
          </p>
        </>
    ...
    <div className={`min-w-0 max-w-full space-y-2 ${settingsCopy}`}>
      ...
      {db && activeSubTab === 'staff' && (canStaff || canStaffTeam) ? (
        <div role="tabpanel" aria-labelledby="tab-staff" className="space-y-3">
          ...
          <StaffManagementView embedded teamScopeOnly={canStaffTeam && !canStaff} />
        </div>
      ) : null}
